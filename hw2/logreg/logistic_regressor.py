...
class LogisticRegressor:
    ...
    def loss(self, *args):
        ...
        ##########################################################################
        h = np.dot(X, theta.T)
        J = 1/m * (np.dot(-y.T, np.log(utils.sigmoid(h))) - np.dot((1 - y).T, np.log(1 - utils.sigmoid(h))))
    ...
    def grad_loss(self, *args):
        ...
        ##########################################################################
        grad = 1/m * np.dot((utils.sigmoid(np.dot(X, theta.T)) - y).T, X)
    ...
    def predict(self, X):
        ...
        y_pred = np.where(utils.sigmoid(np.dot(X, self.theta.T)) > 0.5, 1, 0)
...
class RegLogisticRegressor:
    ...
    def loss(self, *args):
        ...
        J = 1/m * (np.dot(-y, np.log(h)) - np.dot(1-y, np.log(1 - h))) + reg / (2*m) * np.sum(np.square(theta[1:]))
    ...
    def grad_loss(self, *args):
        ...
        grad[1:] = 1/m * np.dot((h - y), X[:, 1:]) + reg / m * theta[1:]
    ...
    def predict(self, X):
        ...
        #                                                                         #
        ###########################################################################
        y_pred = np.where(utils.sigmoid(np.dot(X, self.theta))>0.5, 1, 0)
