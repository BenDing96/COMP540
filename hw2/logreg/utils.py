...
def select_lambda_crossval(X,y,lambda_low,lambda_high,lambda_step,penalty):
    ...
    while lambda_low <= lambda_high:
        ...
        for train_index, test_index in fold.split(X):
            ...
            # insert the a row with one
            XX_train = np.hstack((np.ones([X_train.shape[0], 1], ), X_train))
            XX_test = np.hstack((np.ones([X_test.shape[0], 1], ), X_test))

            if penalty == "l2":
                lin_reg = linear_model.LogisticRegression(penalty=penalty, C=1.0/lambda_low, solver='lbfgs', fit_intercept=True)
                lin_reg.fit(XX_train, y_train)
            else:
                lin_reg = linear_model.LogisticRegression(penalty=penalty, C=1.0/lambda_low, solver='liblinear', fit_intercept=True)
                lin_reg.fit(XX_train, y_train)

            predict = lin_reg.predict(XX_test)
            acc_one = np.mean(predict == y_test)
            acc = acc + acc_one

        averge_acc = acc / 10

        if averge_acc > best_acc:
            best_acc = averge_acc
            best_lambda = lambda_low

        lambda_low = lambda_low + lambda_step
