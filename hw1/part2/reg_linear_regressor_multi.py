...
class RegularizedLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss (self,*args):
        ...
        J = 0.5 * 1/num_examples * (np.sum(diff**2)) + reg * 0.5 * 1/num_examples * np.sum(theta[1:]**2)
    ...
    def grad_loss(self,*args):                                                                          
        ...
        ###########################################################################
        diff = np.dot(X, theta) - y
        grad[0] = 1/num_examples * np.dot(diff, X[:, 0])
        grad[1:] = 1/num_examples * np.dot(diff, X[:, 1:]) + reg / num_examples * theta[1:]
